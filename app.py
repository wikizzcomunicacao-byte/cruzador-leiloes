...
class InformativoLeiloesPDF(FPDF):
  ...
  def footer(self):
    ...
    self.cell(
        0,
        10,
        clean_ascii(f"Informativo de Imoveis em Leilao - Pagina {self.page_no()}"),
        0,
        0,
        "C",
    )
...
def gerar_pdf_informativo(nome_investidor, df_inv):
  ...
  cid_req = (
      str(df_inv["Cidades Solicitadas"].iloc[0]) if not df_inv.empty else "N/A"
  )
  faixa_req = (
    ...
    for tipo in tipos_unicos:
      ...
      for idx, (_, row) in enumerate(df_tipo.iterrows(), start=1):
        ...
        pdf.cell(
            60,
            4,
            clean_ascii(f"Instituição: {row.get('Instituição', 'Judicial/Caixa')}"),
            0,
            1,
        )
        ...
        pdf.cell(
            60,
            4,
            clean_ascii(f"Área do Terreno: {row.get('Área do Terreno', 'N/I')} m²"),
            0,
            0,
        )
        pdf.set_xy(col2_x, current_y)
        ...
        endereco_completo = clean_ascii(
            ...
            f" {row['Cidade Imóvel']}/{row['Estado Imóvel']}"
        )
...
def gerar_excel_profissional(df_input):
  ...
  with pd.ExcelWriter(output, engine="openpyxl") as writer:
    ...
    for col_idx, col_name in enumerate(col_names, start=1):
      ...
      if any(
          k in str(col_name)
          for k in ["Preço", "Avaliação", "Lucro", "Custo"]
      ):
        for c in cells:
  ...
  if "df_final" in st.session_state and not st.session_state["df_final"].empty:
    ...
    with tab1:
      ...
      if not df_filtered.empty:
        ...
        with g_col2:
          ...
          top_cidades = (
              df_filtered["Cidade Imóvel"].value_counts().head(5).reset_index()
          )
    ...
    with tab4:
      ...
      if not df_filtered.empty:
        ...
        st.caption(
            f"Preferências solicitadas:"
            f" {df_inv['Cidades Solicitadas'].iloc[0]} | Faixa:"
            f" {df_inv['Faixa Solicitada'].iloc[0]}"
        )
        st.write(" ")
        ...
        def renderizar_vitrine_com_paginacao(df_investidor):
          ...
          for idx, (_, row) in enumerate(df_paginado.iterrows()):
            ...
            msg_whatsapp = (
                f"Olá {investidor_sel}! Encontrei uma excelente oportunidade de"
                f" leilão para o seu perfil:\n\n🏢 *{row['Título do Imóvel']}*\n📍"
                f" Local: {row['Cidade Imóvel']} -"
                f" {row['Estado Imóvel']}\n💰 Lance Mínimo: R$"
                f" {row['Preço do Leilão (R$)']:,.2f}\n💵 Valor de Avaliação: R$"
                f" {row['Valor de Avaliação (R$)']:,.2f}\n📈 Lucro Líquido"
                f" Estimado: R$ {row['Lucro Líquido Real (R$)']:,.2f}\n🔗"
