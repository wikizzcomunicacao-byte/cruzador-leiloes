...
def gerar_excel_profissional(df_input):
    ...
    # Transforma URLs longas na fórmula de link limpo do Excel
    ...
    with pd.ExcelWriter(output, engine="openpyxl") as writer:
        ...
        # Exibe linhas de grade, congela o cabeçalho e aplica auto-filtro
        ...
        # 2. Formatação das Colunas
        for col_idx, col_name in enumerate(col_names, start=1):
            col_letter = get_column_letter(col_idx)
            cells = ws[col_letter][1:]  # ignora a linha 1 (cabeçalho)
        ...
        # 3. Larguras Personalizadas por Coluna
        col_widths = {
            "ID Investidor": 14,
...
with st.sidebar:
    ...
    file_leiloes = st.file_uploader("1️⃣ Planilha de Leilões (.xlsx)", type=["xlsx", "xls"])
    file_investidores = st.file_uploader("2️⃣ Planilha de Investidores (.xlsx)", type=["xlsx", "xls"])
...
if file_leiloes and file_investidores and executar:
    with st.spinner("Analisando critérios e cruzando bases de dados..."):
        try:
            ...
            for idx, row in df_investidores.iterrows():
                ...
                for _, imovel in sub_sorted.iterrows():
                    resultados.append({
                        ...
                        "Valor de Avaliação (R$)": imovel["Valor de Avaliação do Leiloeiro"],
            ...
            col2.metric("👥 Investidores Atendidos", f"{df_final['Nome do Investidor'].nunique()}" if not df_final.empty else "0")
            col3.metric("🔥 Maior Desconto", (f"{df_final['Desconto (%)'].max():.1f}%" if not df_final.empty else "0%"))
            avg_price = (df_final["Preço do Leilão (R$)"].mean() if not df_final.empty else 0)
            col4.metric("💰 Ticket Médio", f"R$ {avg_price:,.2f}")

            st.markdown("---")

            # ---------------------------------------------------------
            # BOTÃO DE DOWNLOAD COM EXCEL FORMATADO
            # ---------------------------------------------------------
            if not df_final.empty:
                processed_data = gerar_excel_profissional(df_final)
                ...
                st.warning("Nenhum cruzamento encontrado com os critérios atuais.")
